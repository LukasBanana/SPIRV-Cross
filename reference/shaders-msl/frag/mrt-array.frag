...
static inline __attribute__((always_inline))
void write_deeper_in_function(thread unsafe_array<float4,4> (&FragColor), thread float4& vA, thread float4& vB)
{
    FragColor[3] = vA * vB;
}

static inline __attribute__((always_inline))
void write_in_function(thread unsafe_array<float4,4> (&FragColor), thread float4& vA, thread float4& vB)
{
    FragColor[2] = vA - vB;
